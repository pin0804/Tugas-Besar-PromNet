...
<head>
    ...
    <title>Dashboard &mdash; Data Pertanyaan</title>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered first">
                                    <thead>
                                        <tr class="hide_all">
                                            <form method="post" action="dashboard-pertanyaan.php">
                                                <td colspan="3"></td>
                                        ...
                                        <tr class="hide_all">
                                            ...
                                            <td align="left" colspan="4">
                                                <a href="dashboard-pertanyaan.php" title="Refresh Table"><i class="fas fa-redo mr-2"></i>
                                                    Refresh Table</a>
                                            </td>
                                        </tr>
                                        <tr class="hide_all">
                                    ...
                                    <tbody>
                                        ...
                                        if (isset($_POST['btn_cari'])) {
                                            if (!empty($_POST['txt_cari'])) {
                                                $cari = $_POST['txt_cari'];
                                                $sql = "SELECT * FROM tb_pertanyaan WHERE nama LIKE '%" . $cari . "%' 
                                                              OR email LIKE '%" . $cari . "% '
                                                              OR tgl_masuk LIKE '%" . $cari . "% '
                                                              OR status LIKE '%" . $cari . "%'";
                                            } else {
                                                echo "<script>alert('Data Tersebut Tidak Ada !'); window.location='dashboard-pertanyaan.php';</script>";
                                            }
                                        } else {
                                            //Query SQL
                                            $sql = "SELECT * FROM tb_pertanyaan order by tgl_masuk ASC ";
                                        }
                                        $i = 0;
                                        ...
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                if ($row['status'] == "Pending") {
                                                    echo "<td align='center'><p style='color:red;'>" . $row['status'] . "</p></td>";
                                                } else {
                                                    echo "<td align='center'><p style='color:green;'>" . $row['status'] . "</p></td>";
                                                }
                                                echo "<td align='center'>";
                                                echo "<a href='edit-pertanyaan.php?update_id=" . $row['id_pertanyaan'] . "' class='badge badge-pill badge-edit'>
    ...
    <div class="modal fade" id="examplePDF" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <a href="#" class="btn btn-secondary" data-dismiss="modal">Batal</a>
                    <a href="pdf/pertanyaan.php" class="btn btn-primary">Unduh</a>
                </div>
            </div>
        </div>
    </div>
